...
import {describeWithShadows} from "./util";

// All test for the RPC

describeWithShadows("Shadows RPC (Constant)", (context) => {
    it("should have 0 hashrate", async function () {
        expect(await context.web3.eth.getHashrate()).to.equal(0);
    });

    it("should have chainId 888", async function () {
        // The chainId is defined by the Substrate Chain Id, default to 42
        expect(await context.web3.eth.getChainId()).to.equal(888);
    });
