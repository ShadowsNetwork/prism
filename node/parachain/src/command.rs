...
fn load_spec(
	...
	match id {
		"alphanet" => Ok(Box::new(chain_spec::ChainSpec::from_json_bytes(
			&include_bytes!("../../../specs/OmbreAlphaV5.json")[..],
		)?)),
...
impl SubstrateCli for RelayChainCli {
	fn impl_name() -> String {
		"Shadows Parachain Collator".into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		match id {
			"ombre_alpha_relay" => Ok(Box::new(RococoChainSpec::from_json_bytes(
				&include_bytes!("../../../specs/OmbreAlphaV5-Relay.json")[..],
			)?)),
			// If we are not using a shadows-centric pre-baked relay spec, then fall back to the
