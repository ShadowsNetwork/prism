...
pub const SHADOWS_ENGINE_ID: ConsensusEngineId = [b'f', b'r', b'o', b'n'];
...
pub fn find_pre_log<Hash>(
	...
	for log in digest.logs() {
		let log = log.try_to::<PreLog>(OpaqueDigestItemId::PreRuntime(&SHADOWS_ENGINE_ID));
...
pub fn find_post_log<Hash>(
	...
	for log in digest.logs() {
		let log = log.try_to::<PostLog>(OpaqueDigestItemId::Consensus(&SHADOWS_ENGINE_ID));
		match (log, found.is_some()) {
			(Some(_), true) => return Err(FindLogError::MultipleLogs),
			(Some(log), false) => found = Some(log),
			(None, _) => (),
...
pub fn find_log<Hash>(
	...
	for log in digest.logs() {
		let pre_log = log.try_to::<PreLog>(OpaqueDigestItemId::PreRuntime(&SHADOWS_ENGINE_ID));
		...
		let post_log = log.try_to::<PostLog>(OpaqueDigestItemId::Consensus(&SHADOWS_ENGINE_ID));
