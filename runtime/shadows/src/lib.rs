...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("shadows"),
	impl_name: create_runtime_str!("shadows"),
	authoring_version: 1,
	spec_version: 602,
	impl_version: 0,
...
impl pallet_transaction_payment::Trait for Runtime {
	type Currency = Balances;
	type OnTransactionPayment = ShadowTreasury;
...
type OperatorMembershipInstanceShadow = pallet_membership::Instance5;
impl pallet_membership::Trait<OperatorMembershipInstanceShadow> for Runtime {
	type Event = Event;
	type AddOrigin = EnsureRootOrTwoThirdsGeneralCouncil;
	type RemoveOrigin = EnsureRootOrTwoThirdsGeneralCouncil;
	type SwapOrigin = EnsureRootOrTwoThirdsGeneralCouncil;
	type ResetOrigin = EnsureRootOrTwoThirdsGeneralCouncil;
	type PrimeOrigin = EnsureRootOrTwoThirdsGeneralCouncil;
	type MembershipInitialized = ShadowsOracle;
	type MembershipChanged = ShadowsOracle;
}
...
impl pallet_treasury::Trait for Runtime {
	type ModuleId = ShadowTreasuryModuleId;
	type Currency = Balances;
...
impl pallet_staking::Trait for Runtime {
	...
	type RewardRemainder = ShadowTreasury;
	type Event = Event;
	type Slash = ShadowTreasury; // send the slashed funds to the pallet treasury.
...
type ShadowDataProvider = orml_oracle::Instance1;
impl orml_oracle::Trait<ShadowDataProvider> for Runtime {
	type Event = Event;
	type OnNewData = ();
	type CombineData = orml_oracle::DefaultCombineData<Runtime, MinimumCount, ExpiresIn, ShadowDataProvider>;
	type Time = Timestamp;
...
pub struct EnsureRootOrShadowTreasury;
impl EnsureOrigin<Origin> for EnsureRootOrShadowTreasury {
	type Success = AccountId;

	fn try_origin(o: Origin) -> Result<Self::Success, Origin> {
		Into::<Result<RawOrigin<AccountId>, Origin>>::into(o).and_then(|o| match o {
			RawOrigin::Root => Ok(ShadowTreasuryModuleId::get().into_account()),
...
impl orml_vesting::Trait for Runtime {
	...
	type VestedTransferOrigin = EnsureRootOrShadowTreasury;
	type WeightInfo = ();
}
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		ShadowTreasury: pallet_treasury::{Module, Call, Storage, Config, Event<T>},
		...
		// Oracle
		ShadowsOracle: orml_oracle::<Instance1>::{Module, Storage, Call, Config<T>, Event<T>},
		BandOracle: orml_oracle::<Instance2>::{Module, Storage, Call, Config<T>, Event<T>},
		// OperatorMembership must be placed after Oracle or else will have race condition on initialization
		OperatorMembershipShadows: pallet_membership::<Instance5>::{Module, Call, Storage, Event<T>, Config<T>},
...
impl_runtime_apis! {
	...
	> for Runtime {
		fn get_value(provider_id: DataProviderId ,key: CurrencyId) -> Option<TimeStampedPrice> {
			match provider_id {
				DataProviderId::Shadow => ShadowsOracle::get_no_op(&key),
		...
		fn get_all_values(provider_id: DataProviderId) -> Vec<(CurrencyId, Option<TimeStampedPrice>)> {
			match provider_id {
				DataProviderId::Shadow => ShadowsOracle::get_all_values(),
	...
	// benchmarks for shadows modules
	#[cfg(feature = "runtime-benchmarks")]
