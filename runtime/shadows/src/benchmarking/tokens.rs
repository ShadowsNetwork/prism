...
runtime_benchmarks! {
	...
	}: _(RawOrigin::Signed(from), to_lookup, CurrencyId::Token(TokenSymbol::XUSD), amount)
	verify {
		assert_eq!(<Tokens as MultiCurrency<_>>::total_balance(CurrencyId::Token(TokenSymbol::XUSD), &to), amount);
	}
	...
	}: _(RawOrigin::Signed(from.clone()), to_lookup, CurrencyId::Token(TokenSymbol::XUSD))
	verify {
		assert_eq!(<Tokens as MultiCurrency<_>>::total_balance(CurrencyId::Token(TokenSymbol::XUSD), &from), 0);
	}
}
