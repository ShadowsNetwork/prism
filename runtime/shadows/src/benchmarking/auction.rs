use crate::{
	Auction, AuctionId, AuctionManager, AuctionTimeToClose, CurrencyId, DEBTTreasury, Runtime, System, TokenSymbol,
	DOLLARS,
};
...
use module_support::{AuctionManager as AuctionManagerTrait, DEBTTreasury};
...
runtime_benchmarks! {
	...
	bid_surplus_auction_as_first_bidder {
		...
		set_balance(CurrencyId::Token(TokenSymbol::DOS), &bidder, bid_price);
	...
	bid_surplus_auction {
		...
		set_balance(CurrencyId::Token(TokenSymbol::DOS), &bidder, bid_price);
		set_balance(CurrencyId::Token(TokenSymbol::DOS), &previous_bidder, previous_bid_price);
		AuctionManager::new_surplus_auction(surplus_amount)?;
		Auction::bid(RawOrigin::Signed(previous_bidder).into(), auction_id, previous_bid_price)?;
	}: bid(RawOrigin::Signed(bidder), auction_id, bid_price)
