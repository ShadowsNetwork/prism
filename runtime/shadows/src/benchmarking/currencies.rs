...
use crate::{dosExistentialDeposit, AccountId, Amount, Balance, Currencies, CurrencyId, Runtime, TokenSymbol, DOLLARS};
...
runtime_benchmarks! {
	...
	transfer_native_currency_worst_case {
		let dos_existential_deposit = dosExistentialDeposit::get();
		let amount: Balance = dos_existential_deposit.saturating_mul(1000);
		let native_currency_id = CurrencyId::Token(TokenSymbol::DOS);
		let from = account("from", 0, SEED);
	...
	update_balance_native_currency_creating {
		let dos_existential_deposit = dosExistentialDeposit::get();
		let balance: Balance = dos_existential_deposit.saturating_mul(1000);
		let amount: Amount = balance.unique_saturated_into();
		let native_currency_id = CurrencyId::Token(TokenSymbol::DOS);
		let who: AccountId = account("who", 0, SEED);
		let who_lookup = lookup_of_account(who.clone());
