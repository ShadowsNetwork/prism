...
runtime_benchmarks! {
	...
	verify {
		assert_eq!(
			<Currencies as MultiCurrency<_>>::total_balance(CurrencyId::Token(TokenSymbol::DOS), &to),
	...
	verify {
		assert_eq!(
			<Currencies as MultiCurrency<_>>::free_balance(CurrencyId::Token(TokenSymbol::DOS), &to),
