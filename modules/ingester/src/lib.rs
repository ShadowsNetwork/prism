...
pub trait Trait: system::Trait {
	...
	/// The staking currency id, it should be DOT in Shadow.
	type GetStakingCurrencyId: Get<CurrencyId>;

	/// The liquid currency id, it should be LDOT in Shadow.
	type GetLiquidCurrencyId: Get<CurrencyId>;

	/// The origin which may lock and unlock ingester feed to system.
...
impl<T: Trait> PriceProvider<CurrencyId> for Module<T> {
	...
	fn get_price(currency_id: CurrencyId) -> Option<Price> {
		if currency_id == T::GetStableCurrencyId::get() {
			...
			// if is stake_earning liquid currency, return the product of staking currency
			// price and liquid/staking exchange rate.
