...
use support::{
	DEPTTreasury, DEPTTreasuryExtended, EXCHANGEManager, EmergencyShutdown, ExchangeRate, Price, PriceProvider, Rate,
	Ratio, RiskManager,
};
...
pub enum LiquidationStrategy {
	...
	Auction,
	/// Liquidation CDP's collateral by swap with EXCHANGE
	Exchange,
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// The max slippage allowed when liquidate an unsafe CDP by swap with EXCHANGE
			...
			if !T::EmergencyShutdown::is_shutdown() {
				for currency_id in T::CollateralCurrencyIds::get() {
					...
					if !stability_fee_rate.is_zero() && !total_debits.is_zero() {
						...
						if <T as Trait>::DEPTTreasury::on_system_surplus(issued_stable_coin_balance).is_ok() {
	...
	pub fn liquidate_unsafe_cdp(who: T::AccountId, currency_id: CurrencyId) -> DispatchResult {
		...
		let liquidation_strategy = (|| -> Result<LiquidationStrategy, DispatchError> {
			...
			if let Ok(actual_supply_collateral) =
				<T as Trait>::DEPTTreasury::swap_collateral_not_in_auction_with_exact_stable(
					currency_id,
				...
				<T as Trait>::DEPTTreasury::withdraw_collateral(&who, currency_id, refund_collateral_amount)?;

				return Ok(LiquidationStrategy::Exchange);
			}

			// create collateral auctions by cdp treasury
			<T as Trait>::DEPTTreasury::create_collateral_auctions(
				currency_id,
				collateral,
				target_stable_amount,
				who.clone(),
