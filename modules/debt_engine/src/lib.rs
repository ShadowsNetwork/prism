...
// https://github.com/AcalaNetwork/Acala/tree/master/modules/cdp_engine
//! # DEBT Engine Module
//!
//! ## Overview
//!
//! The core module of Mintx protocol. DEBT engine is responsible for handle
//! internal processes about DEBTs, including liquidation, settlement and risk
//! management.
...
use support::{
	DEBTTreasury, DEBTTreasuryExtended, EXCHANGEManager, EmergencyShutdown, ExchangeRate, Price, PriceProvider, Rate,
	Ratio, RiskManager,
};
...
pub trait Trait: SendTransactionTypes<Call<Self>> + system::Trait + lend::Trait {
	...
	/// The default liquidation ratio for all collateral types of DEBT
	...
	/// The default liquidation penalty rate when liquidate unsafe DEBT
	...
	/// The max slippage allowed when liquidate an unsafe DEBT by swap with
	/// EXCHANGE
	type MaxSlippageSwapWithEXCHANGE: Get<Ratio>;

	/// The DEBT treasury to maintain bad debts and surplus generated by DEBTs
	type DEBTTreasury: DEBTTreasuryExtended<Self::AccountId, Balance = Balance, CurrencyId = CurrencyId>;

	/// The price source of all types of currencies related to DEBT
	type PriceSource: PriceProvider<CurrencyId>;

	/// The EXCHANGE participating in liquidation
...
pub enum LiquidationStrategy {
	/// Liquidation DEBT's collateral by create collateral auction
	Auction,
	/// Liquidation DEBT's collateral by swap with EXCHANGE
	Exchange,
}
...
pub struct RiskManagementParams {
	...
	/// DEBT's owner cannot issue more stablecoin under the collateral type.
	...
	/// DEBT under this collateral type is below the liquidation ratio, this
	/// DEBT is unsafe and can be liquidated. `None` value means not set
	pub liquidation_ratio: Option<Ratio>,

	/// Liquidation penalty rate, when liquidation occurs,
	/// DEBT will be deducted an additional penalty base on the product of
	...
	/// DEBT so that the current collateral ratio is lower than the required
...
decl_event!(
	...
	where
		...
		Balance = Balance,
	{
		/// Liquidate the unsafe DEBT. \[collateral_type, owner, collateral_amount, bad_debt_value, liquidation_strategy\]
		LiquidateUnsafeDEBT(CurrencyId, AccountId, Balance, Balance, LiquidationStrategy),
		/// Settle the DEBT has debit. [collateral_type, owner]
		SettleDEBTInDebit(CurrencyId, AccountId),
...
decl_error! {
	...
	pub enum Error for Module<T: Trait> {
		...
		/// The DEBT must be unsafe to be liquidated
		MustBeUnsafe,
		/// Invalid collateral type
		InvalidCollateralType,
		/// Remain debit value in DEBT below the dust amount
		RemainDebitValueTooSmall,
		/// Feed price is invalid
		InvalidFeedPrice,
		/// No debit value in DEBT so that it cannot be settled
		NoDebitValue,
...
decl_storage! {
	trait Store for Module<T: Trait> as DEBTEngine {
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// The minimum debit value allowed exists in DEBT which has debit amount to avoid dust
		...
		/// The max slippage allowed when liquidate an unsafe DEBT by swap with EXCHANGE
		...
		/// Liquidate unsafe DEBT
		///
		/// The dispatch origin of this call must be _None_.
		///
		/// - `currency_id`: DEBT's collateral type.
		/// - `who`: DEBT's owner.
		#[weight = (T::WeightInfo::liquidate_by_exchange(), DispatchClass::Operational)]
		pub fn liquidate(
			origin,
			...
			with_transaction_result(|| {
				...
				Self::liquidate_unsafe_debt(who, currency_id)?;
				Ok(())
			})?;
		}

		/// Settle DEBT has debit after system shutdown
		///
		/// The dispatch origin of this call must be _None_.
		///
		/// - `currency_id`: DEBT's collateral type.
		...
		pub fn settle(
			...
			with_transaction_result(|| {
				...
				Self::settle_debt_has_debit(who, currency_id)?;
				Ok(())
			})?;
		}

		/// Update global parameters related to risk management of DEBT
		///
		...
		/// Update parameters related to risk management of DEBT under specific collateral type
		///
		/// The dispatch origin of this call must be `UpdateOrigin`.
		///
			...
			if !T::EmergencyShutdown::is_shutdown() {
				for currency_id in T::CollateralCurrencyIds::get() {
					...
					if !stability_fee_rate.is_zero() && !total_debits.is_zero() {
						...
						if <T as Trait>::DEBTTreasury::on_system_surplus(issued_stable_coin_balance).is_ok() {
		...
		/// Runs after every block. Start offchain worker to check DEBT and
...
impl<T: Trait> Module<T> {
	...
	fn _offchain_worker() -> Result<(), OffchainErr> {
		...
		while let Some((who, Position { collateral, debit })) = map_iterator.next() {
			if !is_shutdown && Self::is_debt_unsafe(currency_id, collateral, debit) {
				// liquidate unsafe DEBTs before emergency shutdown occurs
				Self::submit_unsigned_liquidation_tx(currency_id, who);
			} else if is_shutdown && !debit.is_zero() {
				// settle DEBTs with debit after emergency shutdown occurs.
	...
	pub fn is_debt_unsafe(currency_id: CurrencyId, collateral: Balance, debit: Balance) -> bool {
	...
	// settle debt has debit when emergency shutdown
	pub fn settle_debt_has_debit(who: T::AccountId, currency_id: CurrencyId) -> DispatchResult {
		let Position { collateral, debit } = <LendOf<T>>::positions(currency_id, &who);
		ensure!(!debit.is_zero(), Error::<T>::NoDebitValue);

		// confiscate collateral in debt to debt treasury
		// and decrease DEBT's debit to zero
		...
		Self::deposit_event(RawEvent::SettleDEBTInDebit(currency_id, who));
		Ok(())
	}

	// liquidate unsafe debt
	pub fn liquidate_unsafe_debt(who: T::AccountId, currency_id: CurrencyId) -> DispatchResult {
		let Position { collateral, debit } = <LendOf<T>>::positions(currency_id, &who);

		// ensure the debt is unsafe
		ensure!(
			Self::is_debt_unsafe(currency_id, collateral, debit),
			Error::<T>::MustBeUnsafe
		);

		// confiscate all collateral and debit of unsafe debt to debt treasury
		...
		// try use collateral to swap enough native token in EXCHANGE when the price
		// impact is below the limit, otherwise create collateral auctions.
		let liquidation_strategy = (|| -> Result<LiquidationStrategy, DispatchError> {
			// swap exact stable with EXCHANGE in limit of price impact
			if let Ok(actual_supply_collateral) =
				<T as Trait>::DEBTTreasury::swap_collateral_not_in_auction_with_exact_stable(
					currency_id,
					target_stable_amount,
					collateral,
					Some(T::MaxSlippageSwapWithEXCHANGE::get()),
				) {
				// refund remain collateral to DEBT owner
				...
				<T as Trait>::DEBTTreasury::withdraw_collateral(&who, currency_id, refund_collateral_amount)?;

				return Ok(LiquidationStrategy::Exchange);
			}

			// create collateral auctions by debt treasury
			<T as Trait>::DEBTTreasury::create_collateral_auctions(
				currency_id,
				collateral,
				target_stable_amount,
				who.clone(),
		...
		Self::deposit_event(RawEvent::LiquidateUnsafeDEBT(
			currency_id,
			who,
			collateral,
			bad_debt_value,
	...
	fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
		match call {
			Call::liquidate(currency_id, who) => {
				let Position { collateral, debit } = <LendOf<T>>::positions(currency_id, &who);
				if !Self::is_debt_unsafe(*currency_id, collateral, debit) || T::EmergencyShutdown::is_shutdown() {
					return InvalidTransaction::Stale.into();
				}

				ValidTransaction::with_tag_prefix("DEBTEngineOffchainWorker")
