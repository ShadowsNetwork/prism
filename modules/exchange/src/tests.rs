...
use mock::{
	DexModule, ExtBuilder, Origin, Runtime, System, TestEvent, Tokens, ALICE, AUSD, AUSD_DOT_PAIR, AUSD_XBTC_PAIR, BOB,
	DOS, DOT, XBTC,
};
...
fn get_target_amounts_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::get_target_amounts(&vec![DOT, AUSD, DOS], 10000, None),
...
fn get_supply_amounts_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::get_supply_amounts(&vec![DOT, AUSD, DOS], 10000, None),
			Error::<Runtime>::TradingPairNotAllowed,
		);
		assert_eq!(
...
fn add_liquidity_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::add_liquidity(Origin::signed(ALICE), DOS, AUSD, 100_000_000, 100_000_000),
			Error::<Runtime>::TradingPairNotAllowed
		);
		assert_noop!(
...
fn do_swap_with_exact_supply_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::do_swap_with_exact_supply(&BOB, &[DOT, DOS], 100_000_000_000_000, 0, None),
			Error::<Runtime>::TradingPairNotAllowed,
		);
...
fn do_swap_with_exact_target_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::do_swap_with_exact_target(&BOB, &[DOT, DOS], 250_000_000_000_000, 200_000_000_000_000, None),
			Error::<Runtime>::TradingPairNotAllowed,
		);
