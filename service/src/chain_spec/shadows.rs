...
fn shadows_session_keys(grandpa: GrandpaId, babe: BabeId) -> shadows_runtime::SessionKeys {
	shadows_runtime::SessionKeys { grandpa, babe }
}
...
pub fn local_testnet_config() -> Result<ChainSpec, String> {
	...
	let wasm_binary = shadows_runtime::WASM_BINARY.ok_or("Dev runtime wasm binary not available")?;
...
pub fn latest_shadows_testnet_config() -> Result<ChainSpec, String> {
	let mut properties = Map::new();
	properties.insert("tokenSymbol".into(), "DOS".into());
	properties.insert("tokenDecimals".into(), 18.into());
	...
	Ok(ChainSpec::from_genesis(
		...
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//oracle"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//1//validator"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//1//babe"
		// ./target/debug/subkey --ed25519 inspect "$SECRET//shadows//1//grandpa"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//2//validator"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//2//babe"
		// ./target/debug/subkey --ed25519 inspect "$SECRET//shadows//2//grandpa"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//3//validator"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//shadows//3//babe"
		// ./target/debug/subkey --ed25519 inspect "$SECRET//shadows//3//grandpa"
		move || {
			shadows_genesis(
				wasm_binary,
				vec![
		...
		vec![
			"/dns/testnet-bootnode-1.shadows.laminar.one/tcp/30333/p2p/12D3KooWAFUNUowRqCV4c5so58Q8iGpypVf3L5ak91WrHf7rPuKz"
				.parse()
				.unwrap(),
		],
...
pub fn shadows_testnet_config() -> Result<ChainSpec, String> {
	ChainSpec::from_json_bytes(&include_bytes!("../../../resources/shadows-dist.json")[..])
}
	...
	shadows_runtime::GenesisConfig {
		...
		pallet_session: Some(SessionConfig {
			keys: initial_authorities
				.iter()
				.map(|x| (x.0.clone(), x.0.clone(), shadows_session_keys(x.2.clone(), x.3.clone())))
				.collect::<Vec<_>>(),
		}),
		...
		pallet_membership_Instance5: Some(OperatorMembershipShadowsConfig {
			members: vec![root_key.clone()],
			phantom: Default::default(),
		}),
		...
		orml_oracle_Instance1: Some(ShadowsOracleConfig {
