...
//! Shadow service. Specialized wrapper over substrate service.
...
pub trait IdentifyVariant {
	...
	fn is_shadows(&self) -> bool;
}

impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_shadows(&self) -> bool {
		self.id().starts_with("shadows") || self.id().starts_with("man")
	}
}

/// Shadow's full backend.
type FullBackend = sc_service::TFullBackend<Block>;

/// Shadow's select chain.
type FullSelectChain = sc_consensus::LongestChain<FullBackend, Block>;

/// Shadow's full client.
type FullClient<RuntimeApi, Executor> = sc_service::TFullClient<Block, RuntimeApi, Executor>;

/// Shadow's full Grandpa block import.
...
pub fn new_partial<RuntimeApi, Executor>(
	...
	PartialComponents<
		...
		(
			impl Fn(shadows_rpc::DenyUnsafe, shadows_rpc::SubscriptionTaskExecutor) -> shadows_rpc::RpcExtension,
			(
...
{
	if !test {
		// If we're using prometheus, use a registry with a prefix of `shadows`.
		if let Some(PrometheusConfig { registry, .. }) = config.prometheus_config.as_mut() {
			*registry = Registry::new_custom(Some("shadows".into()), None)?;
		}
	}
	...
	let rpc_extensions_builder = {
		...
		move |deny_unsafe, subscription_executor| -> shadows_rpc::RpcExtension {
			let deps = shadows_rpc::FullDeps {
				client: client.clone(),
				pool: transaction_pool.clone(),
				select_chain: select_chain.clone(),
				deny_unsafe,
				babe: shadows_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: shadows_rpc::GrandpaDeps {
...
> {
	...
	let PartialComponents {
		...
		task_manager,
		..
	} = new_partial::<shadows_runtime::RuntimeApi, MandalaExecutor>(config, false)?;
...
pub fn build_light(config: Configuration) -> Result<TaskManager, ServiceError> {
	new_light::<shadows_runtime::RuntimeApi, MandalaExecutor>(config).map(|r| r.0)
}
...
pub fn build_full(
	...
	let (task_manager, _, client, _, _, network_status_sinks) =
		new_full::<shadows_runtime::RuntimeApi, MandalaExecutor>(config, test)?;
