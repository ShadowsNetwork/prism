...
if [ -z "$STANDALONE_BINARY" ]; then
  STANDALONE_BINARY="node/standalone/target/release/ombre-standalone"
fi

if [ -z "$PARACHAIN_BINARY" ]; then
  PARACHAIN_BINARY="target/release/ombre-alphanet"
fi

if [ -z "$STANDALONE_SPEC_TMP" ]; then
  STANDALONE_SPEC_TMP="/tmp/ombre-standalone-specs-tmp.json"
fi

if [ -z "$PARACHAIN_SPEC_TEMPLATE" ]; then
  PARACHAIN_SPEC_TEMPLATE="specs/ombre-alphanet-specs-template.json"
fi

if [ -z "$PARACHAIN_SPEC_PLAIN" ]; then
  PARACHAIN_SPEC_PLAIN="$PARACHAIN_BUILD_FOLDER/ombre-alphanet-specs-plain.json"
fi

if [ -z "$PARACHAIN_SPEC_RAW" ]; then
  PARACHAIN_SPEC_RAW="$PARACHAIN_BUILD_FOLDER/ombre-alphanet-specs-raw.json"
fi

if [ -z "$PARACHAIN_SPEC_TMP" ]; then
  PARACHAIN_SPEC_TMP="/tmp/ombre-alphanet-specs-tmp.json"
fi

if [ -z "$PARACHAIN_WASM" ]; then
  PARACHAIN_WASM="$PARACHAIN_BUILD_FOLDER/ombre-alphanet-runtime.wasm"
fi

if [ -z "$PARACHAIN_GENESIS" ]; then
  PARACHAIN_GENESIS="$PARACHAIN_BUILD_FOLDER/ombre-alphanet-genesis.txt"
fi
